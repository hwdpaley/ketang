...
export default class extends Base {
    ...
    async tjAction() {
        ...
        if(think.isEmpty(openid)){
            return this.json({"success":false,"code":130504,"msg":"请在微信中打开，谢谢！","obj":null,"map":null,"list":null});
        }
        let id =this.get('id');
        console.log("id-------"+JSON.stringify(id));
        let data=this.post();
        console.log("data name-------"+JSON.stringify(data));
        // console.log("data phone-------"+JSON.stringify(data.eq[f_phone]));
        return this.json({"success":false,"code":130001,"msg":"谢谢您的参与！呵呵123","obj":null,"map":null,"list":null});
    }
    async logserverAction() {
        ...
        let data=this.post();
        console.log("data-------"+JSON.stringify(data));
        return this.json({"header":{"code":10200,"timing":1,"count":0,"desc":"ok"}});
    }
    async yqx1Action() {
    ...
    async dydemoAction() {
        ...
        let res={"success":true,"code":120504,"msg":"谢谢您的参与！","obj":null,"map":null,"list":null}
        return this.json(res);
        // this.active = ['/', '/yqx', '/lmeOWxwg.html'];
        // return this.display('test1');
    }
